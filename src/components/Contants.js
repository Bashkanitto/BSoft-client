export const stackList = [
	{
		name: 'Kotlin',
		img: 'images/stack1.png',
	},
	{
		name: 'NodeJs',
		img: 'images/stack2.png',
	},
	{
		name: 'Flutter',
		img: 'images/stack3.png',
	},
	{
		name: 'Python',
		img: 'images/stack4.png',
	},
	{
		name: 'React Js',
		img: 'images/stack5.png',
	},
	{
		name: 'Android Studio',
		img: 'images/stack6.png',
	},
];

export const teamList = [
	{
		name: 'Даулет Дауитбек',
		role: 'Ментор по веб разработке',
		img: '/images/imgDaulet.jpg',
		reverse: true,
	},
	{
		name: 'Рауан Жақсыбаев',
		role: 'Ментор по мобильной разработке',
		img: '/images/imgRauan.jpg',
		text: 'Программа "Менторство в IT" предназначена для опытных IT-специалистов, желающих поделиться своими знаниями и опытом через наставничество, а также для тех, кто стремится стать эффективными менторами в технологической сфере. Курс охватывает ключевые аспекты менторства, включая методы коммуникации, стратегии обучения, психологию наставничества и развитие лидерских качеств, необходимых для поддержки и развития IT-специалистов на разных этапах их карьеры.',
		reverse: false,
	},
];

export const goalsList = [
	{
		title: 'Upgrade yourself Today',
		img: 'images/imgGoals.png',
		text: 'В нашей школе программирования мы стремимся вооружить взрослых навыками, необходимыми им для достижения успеха в сегодняшнюю цифровую эпоху. Наше внимание к передовым концепциям веб-разработки и разработки мобильных устройств гарантирует, что наши студенты готовы решать сложные задачи и создавать инновационные решения, которые приносят реальные результаты. С опытными инструкторами и поддерживающим сообществом мы стремимся помочь вам достичь ваших целей и преуспеть в вашей карьере.',
	},
	{
		title: 'Upgrade yourself Today',
		img: 'images/imgGoals.png',
		text: 'В нашей школе программирования мы стремимся вооружить взрослых навыками, необходимыми им для достижения успеха в сегодняшнюю цифровую эпоху. Наше внимание к передовым концепциям веб-разработки и разработки мобильных устройств гарантирует, что наши студенты готовы решать сложные задачи и создавать инновационные решения, которые приносят реальные результаты. С опытными инструкторами и поддерживающим сообществом мы стремимся помочь вам достичь ваших целей и преуспеть в вашей карьере.',
	},
	{
		title: 'Upgrade yourself Today',
		img: 'images/imgGoals.png',
		text: 'В нашей школе программирования мы стремимся вооружить взрослых навыками, необходимыми им для достижения успеха в сегодняшнюю цифровую эпоху. Наше внимание к передовым концепциям веб-разработки и разработки мобильных устройств гарантирует, что наши студенты готовы решать сложные задачи и создавать инновационные решения, которые приносят реальные результаты. С опытными инструкторами и поддерживающим сообществом мы стремимся помочь вам достичь ваших целей и преуспеть в вашей карьере.',
	},
];

export const priceList = [
	{
		title: 'Базовые курсы',
		description: 'Изучение основных навыков программирования',
		img: '/images/imgPrice1.png',
		list: [
			'Офлайн обучение',
			'Свясть с предоподователем',
			'Промежуточные тесты',
			'Домашние задания',
			'Менторство',
		],
		price: '36,000 тенге / месяц',
	},
	{
		title: 'Базовые курсы',
		description: 'Изучение основных навыков программирования',
		img: '/images/imgPrice2.png',
		list: [
			'Офлайн обучение',
			'Свясть с предоподователем',
			'Промежуточные тесты',
			'Домашние задания',
			'Менторство',
		],
		price: '36,000 тенге / месяц',
	},
];

export const reviewsList = [
	{
		name: 'Ержан А.',
		role: 'Фронтенд На React',
		study: 'Студент ATY',
		text: 'Я очень доволен тем, как быстро я научился программированию благодаря курсам BSoft. Учителя были профессионалами в своей области и были всегда готовы помочь мне.',
		review: 3,
	},
	{
		name: 'Ержан А.',
		role: 'Фронтенд На React',
		study: 'Студент ATY',
		text: 'Я очень доволен тем, как быстро я научился программированию благодаря курсам BSoft. Учителя были профессионалами в своей области и были всегда готовы помочь мне.',
		review: '5',
		reverse: true,
	},
	{
		name: 'Ержан А.',
		role: 'Фронтенд На React',
		study: 'Студент ATY',
		text: 'Я очень доволен тем, как быстро я научился программированию благодаря курсам BSoft. Учителя были профессионалами в своей области и были всегда готовы помочь мне.',
		review: '5',
	},
];

export const coursesList = [
	{
		name: 'ReactJs from Zero to Hero',
		img: 'images/imgCourse1.png',
	},
	{
		name: 'NodeJs from Zero to Hero',
		img: 'images/imgCourse2.png',
	},
	{
		name: 'Data Analysis with Python',
		img: 'images/imgCourse3.png',
	},
];

export const questionsList = [
	{
		question: 'Какие Языки Программирования Преподаются?',
		answer:
			'Для некоторых курсов могут потребоваться предварительные знания основ программирования, но школа предлагает вводные курсы, чтобы помочь учащимся освоиться.',
	},
	{
		question: 'Какие курсы доступны',
		answer:
			'Для некоторых курсов могут потребоваться предварительные знания основ программирования, но школа предлагает вводные курсы, чтобы помочь учащимся освоиться.',
	},
	{
		question: 'Какие Предварительные Требования Для Курсов?',
		answer:
			'Для некоторых курсов могут потребоваться предварительные знания основ программирования, но школа предлагает вводные курсы, чтобы помочь учащимся освоиться.',
	},
];
